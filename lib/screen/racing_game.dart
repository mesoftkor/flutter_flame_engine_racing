...
class RacingGame extends FlameGame with TapCallbacks, HasCollisionDetection {
  ...
  //로드 될 이미지(Sprite) 변수 선언
  late Sprite playerSprite;
  late Sprite obstacleSprite;
  late Sprite leftMoveButtonSprite;
  late Sprite rightMoveButtonSprite;

  int currentScore = 0; //현재 표시되는 점수
  late TextComponent scoreText; //스코어 텍스트

  double nextSpawnseconds = 0; // 다음 장애물 생성까지의 시간
  Function onGameOver; //게임 오버가 됐다는 것을 알려주는 콜백함수

  int playerDirection = 0; //플레이어 이동 방향 상태 변수(0:정지, 1: 오른쪽이동, -1:왼쪽이동)

  RacingGame({required this.onGameOver}); //생성자로 gameOver 상황을 콜백
  ...
  Future<void> onLoad() async {
    ...
    lifeHeartList.add(
        LifeHeart(position: Vector2(30, 60), heartSprite: Sprite(heartImg)));
    lifeHeartList.add(
        LifeHeart(position: Vector2(60, 60), heartSprite: Sprite(heartImg)));
    lifeHeartList.add(
        LifeHeart(position: Vector2(90, 60), heartSprite: Sprite(heartImg)));
  ...
  void update(double dt) {
    ...
    if (nextSpawnseconds < 0) {
      add(Obstacle(
          position: Vector2(
              size.x * Random().nextDouble() * 1, 0), //장애물 x축 위치를 랜덤으로 설정
          obstacleSprite: obstacleSprite));
    }

    nextSpawnseconds = 0.3 * Random().nextDouble() * 2; //생성 시간을 랜덤으로 설정
    ...
    if (playerDirection == 1) {
      ...
      player.position = Vector2(
          player.position.x >= size.x - 30 ? player.position.x : player.x + 7,
          player.position.y);
